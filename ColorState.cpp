...
uint8_t ColorState::currentAngle(int16_t current_time_in_minutes){
  if (next_transition >= RISE_PEAK &&  next_transition <= NIGHT_START){
    current_angle = eastern_horizon + static_cast<int>((float)(western_horizon - eastern_horizon)*(current_time_in_minutes - transition_time[NIGHT_END])/(transition_time[NIGHT_START] - transition_time[NIGHT_END]));
    // Serial.print("SUN Angle: ");
    daytime = 1;
  } else if (next_transition == NIGHT_MID){
    current_angle = eastern_horizon + static_cast<int>((float)(western_horizon - eastern_horizon) - (western_horizon - eastern_horizon)/2*(current_time_in_minutes - transition_time[NIGHT_START])/(transition_time[NIGHT_MID] - transition_time[NIGHT_START]));
    // Serial.print("MOON Rising: ");
    daytime = 0;
  } else if (next_transition == NIGHT_END){
    current_angle = eastern_horizon + static_cast<int>((float)(western_horizon - eastern_horizon)/2 - (western_horizon - eastern_horizon)/2*(current_time_in_minutes)/(transition_time[NIGHT_END]));
